...
public class Grid {
	...
	//constructor
	public Grid(int root, int amount, int speed, int maxSteps) {
		...
		Grid.grid = new MyFrame(root, dots, maxSteps, generation);
	...
	public static void updateDots() {
		...
		for (int i=0; i<length; i++) {
			...
			//update inputNeuron of dot 
			dots[i].brain.in.update(dots[i].x, root);
	...
	public static void chooseSurvivors() {
		...
		for (int i=0; i<length; i++) {
			if(Grid.dots[i].x < (Grid.root/4) || Grid.dots[i].x > (Grid.root/4)*3) {
	...
	public static void startNextGeneration() {
		Grid.generation++;
		Grid.grid = new MyFrame(Grid.root, Grid.dots, Grid.maxSteps, Grid.generation);
